// да я спиздил луа из психа, и что вы мне сделаете
...
import funkin.gameplay.GameOverSubstate;
...
class FunkinLua {
	...
	public function new(script:String, raw:String) {
		#if linc_luajit
		lua = LuaL.newstate();
		LuaL.openlibs(lua);
		...
		try{
			// raw = formatLua(raw);
			var result:Dynamic = LuaL.dostring(lua, raw);
			...
			if(resultStr != null && result != 0) {
				...
				#else
				Debug.logError('Error loading lua script: "$script"\n' + resultStr);
				#end
				lua = null;
				return;
			}
		...
		set('songPath', PlayState.SONG.song.toLowerCase());
		...
		set('difficultyPath', difficultyName.toLowerCase());
		...
		set('cameraZoomOnBeat', ClientPrefs.data.cameraZoom);
		...
		Lua_helper.add_callback(lua, "initLuaShader", function(name:String, glslVersion:Int = 120) {
			...
			#else
			Debug.logError("initLuaShader: Platform unsupported for Runtime Shaders!");
			#end
			return false;
		});
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			{
				Debug.logError('setSpriteShader: Shader $shader is missing!');
				return false;
			}
			...
			#else
			Debug.logError("setSpriteShader: Platform unsupported for Runtime Shaders!");
			#end
			return false;
		});
		...
		Lua_helper.add_callback(lua, "getShaderBool", function(obj:String, prop:String) {
			...
			#else
			Debug.logError("getShaderBool: Platform unsupported for Runtime Shaders!");
			Lua.pushnil(lua);
			return null;
			#end
		});
		Lua_helper.add_callback(lua, "getShaderBoolArray", function(obj:String, prop:String) {
			...
			#else
			Debug.logError("getShaderBoolArray: Platform unsupported for Runtime Shaders!");
			Lua.pushnil(lua);
			return null;
			#end
		});
		Lua_helper.add_callback(lua, "getShaderInt", function(obj:String, prop:String) {
			...
			#else
			Debug.logError("getShaderInt: Platform unsupported for Runtime Shaders!");
			Lua.pushnil(lua);
			return null;
			#end
		});
		Lua_helper.add_callback(lua, "getShaderIntArray", function(obj:String, prop:String) {
			...
			#else
			Debug.logError("getShaderIntArray: Platform unsupported for Runtime Shaders!");
			Lua.pushnil(lua);
			return null;
			#end
		});
		Lua_helper.add_callback(lua, "getShaderFloat", function(obj:String, prop:String) {
			...
			#else
			Debug.logError("getShaderFloat: Platform unsupported for Runtime Shaders!");
			Lua.pushnil(lua);
			return null;
			#end
		});
		Lua_helper.add_callback(lua, "getShaderFloatArray", function(obj:String, prop:String) {
			...
			#else
			Debug.logError("getShaderFloatArray: Platform unsupported for Runtime Shaders!");
			Lua.pushnil(lua);
			return null;
			#end
		});
		...
		Lua_helper.add_callback(lua, "setShaderBool", function(obj:String, prop:String, value:Bool) {
			...
			#else
			Debug.logError("setShaderBool: Platform unsupported for Runtime Shaders!");
			#end
		});
		Lua_helper.add_callback(lua, "setShaderBoolArray", function(obj:String, prop:String, values:Dynamic) {
			...
			#else
			Debug.logError("setShaderBoolArray: Platform unsupported for Runtime Shaders!");
			#end
		});
		Lua_helper.add_callback(lua, "setShaderInt", function(obj:String, prop:String, value:Int) {
			...
			#else
			Debug.logError("setShaderInt: Platform unsupported for Runtime Shaders!");
			#end
		});
		Lua_helper.add_callback(lua, "setShaderIntArray", function(obj:String, prop:String, values:Dynamic) {
			...
			#else
			Debug.logError("setShaderIntArray: Platform unsupported for Runtime Shaders!");
			#end
		});
		Lua_helper.add_callback(lua, "setShaderFloat", function(obj:String, prop:String, value:Float) {
			...
			#else
			Debug.logError("setShaderFloat: Platform unsupported for Runtime Shaders!");
			#end
		});
		Lua_helper.add_callback(lua, "setShaderFloatArray", function(obj:String, prop:String, values:Dynamic) {
			...
			#else
			Debug.logError("setShaderFloatArray: Platform unsupported for Runtime Shaders!");
			#end
		});
		...
		Lua_helper.add_callback(lua, "setShaderSampler2D", function(obj:String, prop:String, bitmapdataPath:String) {
			...
			#else
			Debug.logError("setShaderSampler2D: Platform unsupported for Runtime Shaders!");
			#end
		});
			...
			if(doPush)
			{
				if(!ignoreAlreadyRunning)
				{
					for (luaInstance in PlayState.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							Debug.logError('addLuaScript: The script "' + cervix + '" is already running!');
							return;
						}
					}
				}
				var raw:String = null;
				#if (final || !sys)
				raw = Assets.getText(cervix);
				#else
				raw = File.getContent(cervix);
				#end
				PlayState.instance.luaArray.push(new FunkinLua(cervix, raw));
				return;
			}
			Debug.logError("addLuaScript: Script doesn't exist!");
		});
		Lua_helper.add_callback(lua, "removeLuaScript", function(luaFile:String, ?ignoreAlreadyRunning:Bool = false) { //would be dope asf.
			...
			if(doPush)
			{
				if(!ignoreAlreadyRunning)
				{
					for (luaInstance in PlayState.instance.luaArray)
					{
						if(luaInstance.scriptName == cervix)
						{
							//Debug.logError('The script "' + cervix + '" is already running!');
			...
			Debug.logError("removeLuaScript: Script doesn't exist!");
		});
		...
		Lua_helper.add_callback(lua, "runHaxeCode", function(codeToRun:String) {
			...
			try {
				...
				Debug.logError(scriptName + ":" + lastCalledFunction + " - " + e);
			}
			#else
			Debug.logError("runHaxeCode: HScript isn't supported on this platform!");
			#end
		...
		Lua_helper.add_callback(lua, "getPropertyFromGroup", function(obj:String, index:Int, variable:Dynamic) {
			...
			Debug.logError("getPropertyFromGroup: Object #" + index + " from group: " + obj + " doesn't exist!");
			Lua.pushnil(lua);
			return null;
		});
		...
		Lua_helper.add_callback(lua, "getObjectOrder", function(obj:String) {
			...
			Debug.logError("getObjectOrder: Object " + obj + " doesn't exist!");
			return -1;
		});
		Lua_helper.add_callback(lua, "setObjectOrder", function(obj:String, position:Int) {
			...
			Debug.logError("setObjectOrder: Object " + obj + " doesn't exist!");
		});
		...
		Lua_helper.add_callback(lua, "doTweenX", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				Debug.logError('doTweenX: Couldnt find object: ' + vars);
			}
		});
		Lua_helper.add_callback(lua, "doTweenY", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				Debug.logError('doTweenY: Couldnt find object: ' + vars);
			}
		});
		Lua_helper.add_callback(lua, "doTweenAngle", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				Debug.logError('doTweenAngle: Couldnt find object: ' + vars);
			}
		});
		Lua_helper.add_callback(lua, "doTweenAlpha", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				Debug.logError('doTweenAlpha: Couldnt find object: ' + vars);
			}
		});
		Lua_helper.add_callback(lua, "doTweenZoom", function(tag:String, vars:String, value:Dynamic, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				Debug.logError('doTweenZoom: Couldnt find object: ' + vars);
			}
		});
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			...
			if(penisExam != null) {
				...
				Debug.logError('doTweenColor: Couldnt find object: ' + vars);
			}
		});
		...
		Lua_helper.add_callback(lua, "exitSong", function(?skipTransition:Bool = false) {
			...
			CustomFadeTransition.targetCamera = PlayState.instance.camOther;
			if(FlxTransitionableState.skipNextTransIn)
				CustomFadeTransition.targetCamera = null;
		...
		Lua_helper.add_callback(lua, "addLuaSprite", function(tag:String, front:Bool = false) {
			if(PlayState.instance.modchartSprites.exists(tag)) {
				...
				if(!shit.wasAdded) {
					...
					shit.wasAdded = true;
					//Debug.logTrace('added a thing: ' + tag);
				}
			}
		});
		Lua_helper.add_callback(lua, "setGraphicSize", function(obj:String, x:Int, y:Int = 0, updateHitbox:Bool = true) {
			...
			Debug.logError('setGraphicSize: Couldnt find object: ' + obj);
		});
		Lua_helper.add_callback(lua, "scaleObject", function(obj:String, x:Float, y:Float, updateHitbox:Bool = true) {
			...
			Debug.logError('scaleObject: Couldnt find object: ' + obj);
		});
		Lua_helper.add_callback(lua, "updateHitbox", function(obj:String) {
			...
			Debug.logError('updateHitbox: Couldnt find object: ' + obj);
		});
		...
		Lua_helper.add_callback(lua, "setHealthBarColors", function(leftHex:String, rightHex:String) {
			PlayState.instance.healthBar.createFilledBar(CoolUtil.colorFromString(leftHex), CoolUtil.colorFromString(rightHex));
			PlayState.instance.healthBar.updateBar();
		});
		Lua_helper.add_callback(lua, "setTimeBarColors", function(leftHex:String, rightHex:String) {
			PlayState.instance.timeSpectrum.emptyColor = CoolUtil.colorFromString(rightHex);
			PlayState.instance.timeSpectrum.fillColor = CoolUtil.colorFromString(leftHex);
		});
		...
		Lua_helper.add_callback(lua, "setObjectCamera", function(obj:String, camera:String = '') {
			...
			Debug.logError("setObjectCamera: Object " + obj + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setBlendMode", function(obj:String, blend:String = '') {
			...
			Debug.logError("setBlendMode: Object " + obj + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "screenCenter", function(obj:String, pos:String = 'xy') {
			...
			Debug.logError("screenCenter: Object " + obj + " doesn't exist!");
		});
		...
		/*Lua_helper.add_callback(lua, "startDialogue", function(dialogueFile:String, music:String = null) {
			...
			Debug.logError('startDialogue: Trying to load dialogue: ' + path);
			...
			#end
			{
				...
				if(shit.dialogue.length > 0) {
					...
					Debug.logError('startDialogue: Successfully loaded dialogue');
					return true;
				} else {
					Debug.logError('startDialogue: Your dialogue file is badly formatted!');
				}
			} else {
				Debug.logError('startDialogue: Dialogue file not found');
		...
		Lua_helper.add_callback(lua, "startVideo", function(videoFile:String) {
			...
			if(FileSystem.exists(Paths.video(videoFile))) {
				...
				Debug.logError('startVideo: Video file not found: ' + videoFile);
			}
			return false;
		...
		Lua_helper.add_callback(lua, "debugPrint", function(text1:Dynamic = '', text2:Dynamic = '', text3:Dynamic = '', text4:Dynamic = '', text5:Dynamic = '') {
			...
			if (text4 == null) text4 = '';
			if (text5 == null) text5 = '';
			Debug.logTrace('' + text1 + text2 + text3 + text4 + text5);
		});
		...
		Lua_helper.add_callback(lua, "setTextString", function(tag:String, text:String) {
			...
			Debug.logError("setTextString: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextSize", function(tag:String, size:Int) {
			...
			Debug.logError("setTextSize: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextWidth", function(tag:String, width:Float) {
			...
			Debug.logError("setTextWidth: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextBorder", function(tag:String, size:Int, color:String) {
			...
			Debug.logError("setTextBorder: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextColor", function(tag:String, color:String) {
			...
			Debug.logError("setTextColor: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextFont", function(tag:String, newFont:String) {
			...
			Debug.logError("setTextFont: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextItalic", function(tag:String, italic:Bool) {
			...
			Debug.logError("setTextItalic: Object " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setTextAlignment", function(tag:String, alignment:String = 'left') {
			...
			Debug.logError("setTextAlignment: Object " + tag + " doesn't exist!");
			return false;
		});
		...
		Lua_helper.add_callback(lua, "getTextString", function(tag:String) {
			...
			Debug.logError("getTextString: Object " + tag + " doesn't exist!");
			Lua.pushnil(lua);
			return null;
		});
		Lua_helper.add_callback(lua, "getTextSize", function(tag:String) {
			...
			Debug.logError("getTextSize: Object " + tag + " doesn't exist!");
			return -1;
		});
		Lua_helper.add_callback(lua, "getTextFont", function(tag:String) {
			...
			Debug.logError("getTextFont: Object " + tag + " doesn't exist!");
			Lua.pushnil(lua);
			return null;
		});
		Lua_helper.add_callback(lua, "getTextWidth", function(tag:String) {
			...
			Debug.logError("getTextWidth: Object " + tag + " doesn't exist!");
			return 0;
		});
		...
		/*Lua_helper.add_callback(lua, "initSaveData", function(name:String, ?folder:String = 'psychenginemods') {
			...
			Debug.logError('initSaveData: Save file already initialized: ' + name);
		});
		Lua_helper.add_callback(lua, "flushSaveData", function(name:String) {
			...
			Debug.logError('flushSaveData: Save file not initialized: ' + name);
		});
		Lua_helper.add_callback(lua, "getDataFromSave", function(name:String, field:String, ?defaultValue:Dynamic = null) {
			...
			Debug.logError('getDataFromSave: Save file not initialized: ' + name);
			return defaultValue;
		});
		Lua_helper.add_callback(lua, "setDataFromSave", function(name:String, field:String, value:Dynamic) {
			...
			Debug.logError('setDataFromSave: Save file not initialized: ' + name);
		});*/
		...
		Lua_helper.add_callback(lua, "saveFile", function(path:String, content:String, ?absolute:Bool = false)
		{
			try {
				...
				Debug.logError("saveFile: Error trying to save " + path + ": " + e, false, false, FlxColor.RED);
			}
			return false;
		});
		Lua_helper.add_callback(lua, "deleteFile", function(path:String, ?ignoreModFolders:Bool = false)
		{
			try {
				...
				Debug.logError("deleteFile: Error trying to delete " + path + ": " + e);
			}
			return false;
		});
		...
		Lua_helper.add_callback(lua, "objectPlayAnimation", function(obj:String, name:String, forced:Bool = false, ?startFrame:Int = 0) {
			Debug.logTrace("objectPlayAnimation is deprecated! Use playAnim instead");
			if(PlayState.instance.getLuaObject(obj,false) != null) {
		...
		Lua_helper.add_callback(lua, "characterPlayAnim", function(character:String, anim:String, ?forced:Bool = false) {
			Debug.logTrace("characterPlayAnim is deprecated! Use playAnim instead");
			switch(character.toLowerCase()) {
				case 'dad':
		...
		Lua_helper.add_callback(lua, "luaSpriteMakeGraphic", function(tag:String, width:Int, height:Int, color:String) {
			Debug.logTrace("luaSpriteMakeGraphic is deprecated! Use makeGraphic instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByPrefix", function(tag:String, name:String, prefix:String, framerate:Int = 24, loop:Bool = true) {
			Debug.logTrace("luaSpriteAddAnimationByPrefix is deprecated! Use addAnimationByPrefix instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
		...
		Lua_helper.add_callback(lua, "luaSpriteAddAnimationByIndices", function(tag:String, name:String, prefix:String, indices:String, framerate:Int = 24) {
			Debug.logTrace("luaSpriteAddAnimationByIndices is deprecated! Use addAnimationByIndices instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
		...
		Lua_helper.add_callback(lua, "luaSpritePlayAnimation", function(tag:String, name:String, forced:Bool = false) {
			Debug.logTrace("luaSpritePlayAnimation is deprecated! Use playAnim instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
		...
		Lua_helper.add_callback(lua, "setLuaSpriteCamera", function(tag:String, camera:String = '') {
			Debug.logTrace("setLuaSpriteCamera is deprecated! Use setObjectCamera instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
				PlayState.instance.modchartSprites.get(tag).cameras = [cameraFromString(camera)];
				return true;
			}
			Debug.logError("Lua sprite with tag: " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "setLuaSpriteScrollFactor", function(tag:String, scrollX:Float, scrollY:Float) {
			Debug.logTrace("setLuaSpriteScrollFactor is deprecated! Use setScrollFactor instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
		...
		Lua_helper.add_callback(lua, "scaleLuaSprite", function(tag:String, x:Float, y:Float) {
			Debug.logTrace("scaleLuaSprite is deprecated! Use scaleObject instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var shit:ModchartSprite = PlayState.instance.modchartSprites.get(tag);
				shit.scale.set(x, y);
				shit.updateHitbox();
		...
		Lua_helper.add_callback(lua, "getPropertyLuaSprite", function(tag:String, variable:String) {
			Debug.logTrace("getPropertyLuaSprite is deprecated! Use getProperty instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var killMe:Array<String> = variable.split('.');
				if(killMe.length > 1) {
		...
		Lua_helper.add_callback(lua, "setPropertyLuaSprite", function(tag:String, variable:String, value:Dynamic) {
			Debug.logTrace("setPropertyLuaSprite is deprecated! Use setProperty instead");
			if(PlayState.instance.modchartSprites.exists(tag)) {
				var killMe:Array<String> = variable.split('.');
				if(killMe.length > 1) {
					var coverMeInPiss:Dynamic = Reflect.getProperty(PlayState.instance.modchartSprites.get(tag), killMe[0]);
			...
			Debug.logError("setPropertyLuaSprite: Lua sprite with tag: " + tag + " doesn't exist!");
			return false;
		});
		Lua_helper.add_callback(lua, "musicFadeIn", function(duration:Float, fromValue:Float = 0, toValue:Float = 1) {
			FlxG.sound.music.fadeIn(duration, fromValue, toValue);
			Debug.logTrace('musicFadeIn is deprecated! Use soundFadeIn instead.');

		});
		Lua_helper.add_callback(lua, "musicFadeOut", function(duration:Float, toValue:Float = 0) {
			FlxG.sound.music.fadeOut(duration, toValue);
			Debug.logTrace('musicFadeOut is deprecated! Use soundFadeOut instead.');
		});
		...
		Lua_helper.add_callback(lua, "startTween", function(tag:String, vars:String, values:Dynamic = null, duration:Float, options:Dynamic = null) {
			...
			if(variables.length > 1) penisExam = getVarInArray(getPropertyLoopThingWhatever(variables), variables[variables.length-1]);

			if (penisExam != null) {
				if(values != null) {
					var myOptions:Dynamic = {
					...
					Debug.logError('startTween: No values on 2nd argument!');
				}
			} else {
				Debug.logError('startTween: Couldnt find object: ' + vars);
			}
		});
	...
	function cameraFromString(cam:String):FlxCamera {
		switch(cam.toLowerCase()) {
			case "camtime" | "time": return PlayState.instance.camTime;
		...
		try {
			...
			if (type != Lua.LUA_TFUNCTION) {
				if (type > Lua.LUA_TNIL)
					Debug.logError("ERROR (" + func + "): attempt to call a " + typeToString(type) + " value");
			...
			if (status != Lua.LUA_OK) {
				...
				Debug.logError("ERROR (" + func + "): " + error);
				return Function_Continue;
			}
	...
	// нахуй комментарии
	public static function formatLua(str:String):String {
		var returnVal:String = str;
		while (returnVal.contains("--[[")) {
			var commStart:Int = returnVal.indexOf("--[[");
			var commEnd:Int = returnVal.indexOf("]]", commStart) + 2;
			var comment:String = returnVal.substring(commStart, commEnd);

			returnVal = returnVal.replace(comment, "");
		}
		while (returnVal.contains("--")) {
			var commStart:Int = returnVal.indexOf("--");
			var commEnd:Int = returnVal.indexOf("\n", commStart);
			var comment:String = returnVal.substring(commStart, commEnd);

			returnVal = returnVal.replace(comment, "");
		}
		Debug.logTrace(returnVal);
		return returnVal;
	}
}
