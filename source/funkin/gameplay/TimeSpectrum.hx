...
class TimeSpectrum extends FlxSpriteGroup {
    ...
    public function new() {
        ...
        barShit = new FlxSprite().makeGraphic(1, 1, 0x00000000);
    }
    ...
    public function updateWaveform(elapsed:Float) {
        ...
        for (i in 0...Std.int(FlxG.width / 19)) {
            ...
            // if (spec > lerpSpectrum[i])
                lerpSpectrum[i] = CoolUtil.coolLerp(spec, lerpSpectrum[i], 0.8);
            // else
                // lerpSpectrum[i] = CoolUtil.coolLerp(spec, lerpSpectrum[i], 1);
