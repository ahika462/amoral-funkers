...
class Channel {
	...
	public function onComplete() {
		completed = true;
		#if !prod
		//Debug.logTrace("onComplete " + haxe.Timer.stamp() );
		#end
		//stop();
		if( onEnd!=null ) {
			var cb = onEnd;
	...
	public function isComplete(){
		#if !prod
		if ( completed ){
			//Debug.logTrace("already completed");
		}
		
		if ( started ){
			//Debug.logTrace("started ok");
		}
		#end
...
class ChannelEventInstance extends Channel {//basically a sound instance
	...
	public override function isPlaying(){
		...
		if ( data == null ) {
			//#if !prod
			//Debug.logTrace("[CEI]no data " + name);
			//#end
			return false;
		}
	...
	public override function setPlayCursorMs(posMs:Float) {
		...
		if ( res != FMOD_OK){
			#if debug
			//Debug.logTrace("[SND][Channel]{"+name+"} Repositionning S err " + FaxeRef.fmodResultToString(res)+" to :"+pos+" ("+posMs+")");
			#end
		}
		
		#if debug
		//Debug.logTrace("setPlayCursorMs "+posMs);
		#end
	}
	...
	public override function getVolume() : Float{
		...
		if ( res != FMOD_OK){
			#if debug
			//Debug.logTrace("[SND][Channel]{"+name+"} getVolume err " + FaxeRef.fmodResultToString(res));
			#end
		}
		return vol;
	}
	...
	public override function setVolume(v:Float){
		if (data == null ){
			//#if debug
			//Debug.logTrace("no data for "+name);
			//#end
			return;
		}
		...
		if ( res != FMOD_OK){
			//#if debug
			//Debug.logTrace("[SND][Channel]{"+name+"} setVolume err " + FaxeRef.fmodResultToString(res));
			//#end
		}
		else {
			//if ( isDebug ){
				//#if !prod
				//Debug.logTrace("[SND][Channel]{"+name+"} setVolume ok " + v);
				//#end
			//}
		}
	}
...
class ChannelLowLevel extends Channel{
	...
	public override function isPlaying(){
		...
		if (data == null) {
			//#if !prod
			//Debug.logTrace("no data no playing! "+name);
			//#end
			return false;
		}
		var b : Bool = false;
		...
		if ( res != FMOD_OK ){
			//#if debug
			//Debug.logTrace("[SND][ChannelLowLevel]{"+name+"} isPlaying err " + FaxeRef.fmodResultToString(res));
			//#end
			return false;
		}
		return b;
	...
	public override function setPlayCursorMs(posMs:Float) {
		...
		if ( res != FMOD_OK){
			//#if debug
			//Debug.logTrace("[SND][Channel]{"+name+"} Repositionning S err " + FaxeRef.fmodResultToString(res)+" to :"+posU+" ("+posMs+")");
			//#end
		}
	}
	...
	public override function setVolume(v:Float){
		if (data == null) {
			//if ( isDebug ){
				//#if !prod
				//Debug.logTrace("[SND][Channel]{"+name+"} setVolume no data");
				//#end
			//}
			return;
		}
		...
		if ( res != FMOD_OK){
			...
			if ( isDebug ){
				//#if !prod
				//Debug.logTrace("[SND][Channel]{"+name+"} setVolume ok " + v+" corrected:"+vcl);
				//#end
			}
		}
	}
...
class SoundLowLevel extends Sound{
	...
	override function get_length() : Float{
		...
		if ( res != FMOD_OK ){
			#if debug
			//Debug.logTrace("impossible to retrieve sound len");
			#end
		}
	...
	public override function play( ?offsetMs : Float = 0.0, ?nbLoops:Int = 1, ?volume:Float = 1.0) : Channel {
		...
		#if debug
		//Debug.logTrace("[Sound] offset " + offsetMs);
		//Debug.logTrace("play " + haxe.Timer.stamp() );
		#end
...
class Snd {
	...
	public function start(loops:Int=0, vol:Float=1.0, ?startOffsetMs:Float=0.0) {
		if ( DISABLED ) 			{
			//#if debug
			//Debug.logTrace("[SND] Disabled");
			//#end
			return;
		}
		if ( sound == null ){
			//#if debug
			//Debug.logTrace("[SND] no inner sound");
			//#end
			return;
		}
		...
		if ( isPlaying() ){
			//#if !prod
			//Debug.logTrace(name+" interrupting ");
			//#end
	...
	public static function dumpMemory(){
		...
		if ( res != FMOD_OK){
			//#if debug
			//Debug.logTrace("[SND] cannot fetch snd ram dump ");
			//#end
		}
	...
	public function fadePlay(?fadeDuration = 100, ?endVolume:Float=1.0 ) {
		var p = play(0.0001);
		if ( p == null ){
			//Debug.logTrace("nothing ret");
		}
		else {
			if ( p.curPlay == null){
				//Debug.logTrace("no curplay wtf?");
			}
			else 
			{
				//Debug.logTrace("curplay ok");
			}
		}
	...
	public function fadeStop( ?fadeDuration = 100 ) {
		if ( !isPlaying()){
			//#if !prod
			//Debug.logTrace("not playing " + name+" winn not unfade");//can cause reentrancy issues
			//#end
			return null;
		}
	...
	public static function release(){
		...
		PLAYING.hardReset();
		released = true;
		//Debug.logTrace("releasing fmod");
		Faxe.fmod_release();
		#if !prod
		Debug.logTrace("fmod released");
		#end
	}
	...
	function refresh() {
		if ( curPlay != null ) {
			...
			//Debug.logTrace("r:"+vol);
			curPlay.setVolume( vol );
		}
		else {
			//#if debug
			//Debug.logTrace("[Snd] no playin no refresh "+name);
			//#end
		}
	}
	...
	public function	setPlayCursorSec( pos:Float ) 	{
		if (curPlay != null)	{
			...
			//#if debug
			//Debug.logTrace("setPlayCursorSec/no current instance");
			//#end
		}
	}
	...
	public function	setPlayCursorMs( pos:Float ){ 
		if (curPlay != null) 	
			...
			//#if debug
			//Debug.logTrace("setPlayCursorMs/no current instance");
			//#end
		}
	}
	...
	public function tweenVolume(v:Float, ?easing:h2d.Tweenie.TType, ?milliseconds:Float=100) : TweenV {
		...
		//#if !prod 
		//Debug.logTrace("tweening " + name+" to " + v);
		//#end
		return t;
	}
	...
	static var _stop = function(t:TweenV){
		#if !prod
		//if( t.parent != null )
			//Debug.logTrace(t.parent.name+" cbk stopped");
		//else 
			//Debug.logTrace(" unbound stop called");
		#end
		t.parent.stop();
	}
	...
	public function isComplete(){
		if ( curPlay == null ) {
			//#if!prod
			//Debug.logTrace("comp: no cur play");
			//#end
			return true;
		}
	...
	public static function loadSound( path:String, streaming : Bool, blocking : Bool  ) : Sound {
		...
		if ( released ) {
			//#if(!prod)
			//Debug.logTrace("FMOD not active "+path);
			//#end
			return null;
		}
		...
		if( DEBUG_TRACK) Debug.logTrace("Snd:loading " + path);
		...
		if ( res != FMOD_OK){
			#if(!prod)
			Debug.logTrace("unable to load " + path + " code:" + res+" msg:"+FaxeRef.fmodResultToString(res));
			#end
			return null;
		}
	...
	public static function loadEvent( path:String ) : Sound {
		...
		if( DEBUG_TRACK) Debug.logTrace("Snd:loadingEvent " + path);
		...
		if ( !ev.isLoaded() ){
			...
			#if debug
			//Debug.logTrace("time to preload:" + (t1 - t0));
			#end
		}
	...
	public static function fromFaxe( path:String ) : Snd {
		...
		if ( s == null){
			#if (!prod)
			Debug.logTrace("unable to find " + path);
			#end
			return null;
		}
	...
	public static function load( path:String, streaming=false,blocking=true ) : Snd {
		...
		if ( s == null) {
			#if !prod
			Debug.logTrace("no such file " + path);
			#end
			return null;
		}
		return new Snd( s,  s.name);
	...
	public static function update() {
		for ( p in PLAYING.backWardIterator())
			if ( p.isComplete()){
				#if !prod
				//Debug.logTrace("[Snd] isComplete " + p);
				#end
				p.onComplete();
			}
	...
	public static function loadSingleBank( filename : String ) : Null<faxe.Faxe.FmodStudioBankRef>{
		if ( released ) {
			#if debug 
			Debug.logTrace("FMOD not active "+filename);
			#end
			return null;
		}
		...
		if ( filename.endsWith(".fsb")) {
			#if debug 
			Debug.logTrace("fsb files not supported");
			#end
		...
		//Debug.logTrace("trying to load " + filename);
		...
		if (result != FMOD_OK)	{
			#if debug
			Debug.logTrace("FMOD failed to LOAD sound bank with errcode:" + result + " errmsg:" + FaxeRef.fmodResultToString(result) + "\n");
			#end
			return null;
		}
		//else 
		//	Debug.logTrace("loading...");
			
		var t1 = haxe.Timer.stamp();
		#if debug
		//Debug.logTrace("time to load bank:" + (t1 - t0)+"s");
		#end
		return cast fbank;
	}
}